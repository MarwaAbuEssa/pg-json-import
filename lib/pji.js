...
pg.connect(options.connection, (err, client, done) => {
        ...
        return _.map(data, function(val,key){  
         ...
         _.each(val[options.relatedtable],(innertbl,key) =>{ 
             // add key to inner table object
               _.extend(innertbl, {[options.foreigkeys]: key});
           })


             return _.map(val[options.relatedtable], function(fval,fkey){
                rows.push(_.extend(parentkey,fval));
 ...
 // get values of columns in json file 
      let values =  cols.map((col) => {
        return  jsonPath({json: row, path: "$.." + col , wrap :true}).length >0 ?  jsonPath({json: row, path: "$.." + col,wrap :true})[0].toString().replace(',', '\,') :null;
      });
