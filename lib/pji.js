...
pg.connect(options.connection, (err, client, done) => {
  ...
  .then((rows) => {
    ...
    return Promise.all(rows.map((row) => {
      let cols = fieldsArr.length ? fieldsArr : Object.keys(row);
     // if(fieldsArr.length){
      //   cols =fieldsArr;
         // _.intersection(Object.keys(row), fieldsArr );
     // }
      let values = cols.map((col) => objectPath.has(row, col)? objectPath.get(row, col):'');
      //  row[col]
      
      
      console.log(cols , 'cols')
      console.log(values , 'values')
       return executeQuery(client, format(queryTemplate, options.table, cols, pg_escape_string(values)));
    }));
  })
