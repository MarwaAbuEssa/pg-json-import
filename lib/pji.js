...
pg.connect(options.connection, (err, client, done) => {
  ...
  .then((rows) => {
    if(options.table == 'photos' || options.table == 'videos' || options.table == 'audios' || options.table == 'posts' ){
    ...
    return Promise.all(rows.map((row) => {
     let cols = fieldsArr.length>0 ? fieldsArr:Object.keys(row);
      ...
      let values =  cols.map((col) => {
       ...
       if((options.table == 'photos' || options.table == 'videos' || options.table == 'audios' ||  options.table == 'postcomments' ||  options.table == 'postapplauses') && col =='createdAt'){
