...
pg.connect(options.connection, (err, client, done) => {
            ...
            let parentkey = _.extend(val, {[options.key]: key});
              _.each(val[options.relatedtable],(innertbl,key) =>{
                ...
                if(options.relatedtable == 'comments') {
                  ...
                  clonestudiouser = _.clone(_.extend(parentkey,_.extend({'user' :key }, {'timestamp':innertbl })));
                }else{
                   clonestudiouser = _.clone(_.extend(parentkey,_.extend(innertbl, {[options.foreigkeys]: key})));
                }
 ...
 //   if(options.table == 'photos' || options.table == 'videos'|| options.table == 'audios' || options.table == 'posts' || options.table == 'postcomments' ||  options.table == 'postapplauses'){
   if(_.contains(['photos' ,'videos' ,'audios','posts','postcomments','postapplauses'], options.table)){
        ...
        _.each(rows,(tblrow) =>{
         // if(tblrow['studioKey'] == "-K3rd4C81__oO4CpRm6q"){
            ...
            if(_.has(row, options.foreigkeys) && !_.contains(relatedkeys, row['userKey'])){
              row['userKey'] =  (row.settings.studio_owner && _.contains(relatedkeys, row.settings.studio_owner))? row.settings.studio_owner : null ;
            }
            filteredrows.push(row);
        //  }
        })
  ...
  .then((rows) => {
   ...
   // if(options.table == 'studioUsers' || options.table == 'photos' || options.table == 'videos' || options.table == 'audios' || options.table == 'posts' ){
     if(_.contains(['studiousers','photos' ,'videos' ,'audios','posts'], options.table)){ 
     ...
     let cols = fieldsArr.length>0 ? fieldsArr:Object.keys(row);
      ...
      let values =  cols.map((col) => {
        if(options.table == 'posts' && col =='created_at'){  
          ...
          return  valu.length >0 ? valu[0] ? new Date(valu[0]) :new Date() :new Date();
          }
          if(options.table == 'posts' && col =='updated_at'){
            let  valu = jsonPath({json: row, path: "$.." + col , wrap :true});
            return  valu.length >0 ? valu[0] ? new Date(Number(valu[0].toString().replace('-',''))) :new Date() :new Date();
           }
           
       if((_.contains(['photos' ,'videos' ,'audios','postcomments','postapplauses'], options.table)) && col =='updatedAt'){
       //if((options.table == 'photos' || options.table == 'videos' || options.table == 'audios' ||  options.table == 'postcomments' ||  options.table == 'postapplauses') && col =='updatedAt'){
          let  valu = jsonPath({json: row, path: "$..timestamp" });
          return  valu.length >0 ? valu[0] ? new Date(valu[0]) :new Date() :new Date();
        }else{
        let  valu = jsonPath({json: row, path: "$.." + col , wrap :true});
